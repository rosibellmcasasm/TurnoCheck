...
```
REFERENCIA-MANDATO        La dio el USUARIO ("quiero que se vea así"). → FIDELIDAD.
...
### ANTES de cerrar la ficha: las 3 opciones A/B/C (obligatorio también CON referencia)

La extracción NO cierra la ficha directo. Con la tabla extraída, se ejecuta **EL PROTOCOLO A/B/C
del `54`**: renderizar 3 **INTERPRETACIONES FIELES** del contrato — la paleta, el mood y la
tipografía extraídas se respetan en las TRES; divergen OBLIGATORIAMENTE en COMPOSICIÓN de la
pantalla y DISPOSITIVO OWNABLE (+ densidad, profundidad, cards, microdetalles) — entregadas
como **página comparativa que el usuario abre y VE** (`direcciones-abc.html` autocontenido o
`/dev/direcciones` — NUNCA solo texto, la ruta va en el mensaje), y **esperar la elección del
usuario** (elige / combina / pide otras 3 / ajusta un detalle). Gates del `54`: TEST DE
DIVERGENCIA (en escala de grises las 3 se distinguen por COMPOSICIÓN, no por color) + fuentes
verificadas en el screenshot (fallback-trampa) + mockups llenos al nivel showcase. Ninguna
...
```
1. ¿El usuario dio referencia visual?
   SÍ → es un CONTRATO: extráela con la TABLA DE EXTRACCIÓN de arriba (mirando la imagen,
        no recordando la descripción). La capa anti-IA solo filtra lo que la imagen no decide.
   NO → LO QUE YA FUNCIONA (PASO 0.2bis): investiga las 3-5 apps LÍDERES del nicho + 1-2
        ...
        con: acento perturbado (hue ±10-25°) + 1 dispositivo ownable + copy. PROHIBIDO inventar
...
2. → 3 opciones A/B/C renderizadas (54, SIEMPRE) → el usuario elige/combina → ficha.
   ...
   Gate: TEST DE DIVERGENCIA (en grises, 3 diseños distintos — no 1 pantalla con 3 acentos).
...
**Si el usuario NO dio referencia visual, tu PRIMERA acción es INVESTIGAR:**
...
Y se DIFERENCIA solo con 3 palancas:
  1. ACENTO PROPIO: el hue del líder PERTURBADO ±10-25° (regla dura de 29)
  2. UN dispositivo ownable (banco del 54 o propio, compatible con lo que los líderes hacen)
...
(qué líder pesa más, hacia dónde perturbar el acento) — no la sustituyen.
...
LA FUSIÓN de varias es EL MÉTODO (0.2bis): de cada líder se toma un subsistema entero
  ...
  y la diferencia la ponen la mezcla + el acento perturbado + el dispositivo propio + el copy.
Y si la referencia la dio el USUARIO, es referencia-MANDATO: clonar su paleta y su mood es lo
  correcto y lo esperado — ver el protocolo REFERENCIA=CONTRATO al inicio de este archivo.
```
...
> 1. **Usa la TABLA DE LÍDERES que ya llenaste en el PASO 0.2bis** (si te la saltaste, vuelve: es la primera acción obligatoria) y aplica su LÓGICA de color extraída: ¿claro o oscuro? ¿una nota o multicolor? ¿qué color = éxito/progreso? ¿degradé y dónde? Hay convenciones de género que el usuario ESPERA y que comunican "esto es del nicho X". La familia cromática ES la de la líder con mejor color, con el hue perturbado ±10-25° — no una familia inventada.
...
**OUTPUT — y el paso que falta ANTES de cerrar la ficha: las 3 opciones A/B/C.** Con los PASOS
0.1–0.7 derivados, NO se fija una sola dirección: se ejecuta **EL PROTOCOLO A/B/C del `54`
(obligatorio SIEMPRE)** — 3 FUSIONES distintas desde la TABLA DE LÍDERES (qué líder pesa más,
...
> la tabla clara de `29` / la matriz audiencia×nicho CON perturbación de hex (regla dura de 29)
...
> proyecto anterior. Si coinciden → están vetados: perturbar el hue o cambiar de candidata
> tipográfica antes de seguir.
