...
export default async function handler(req, res) {
  ...
  const { error: dupErr } = await supabase
    .from('processed_events') // la MISMA tabla de la implementación real de abajo (event_id text primary key, ...)
...
1. ANTES DEL CHECKOUT: la app genera un anon_id (uuid en localStorage, junto al estado
   versionado {v:1, respuestas, plan} — ver 26 → "USUARIO ANÓNIMO → CUENTA") y arma la URL:
   https://pay.hotmart.com/PRODUCTO?email=<si lo pidió el onboarding>&sck=<anon_id>
   (`sck` viaja en el payload del webhook como parámetro de tracking; `?email=` pre-rellena
   el checkout — las mismas dos mitigaciones del bug de email del 2B.)
2. EL WEBHOOK crea la cuenta (rama `!existing` → createUser passwordless + email_confirm: true)
...
| `SUBSCRIPTION_CANCELLATION` / `PURCHASE_EXPIRED` | `cancelled` | **COMPLETO hasta fin de ciclo**, luego free | Cancelación explícita, o expiración tras agotar los reintentos de cobro. No borrar datos (gracia + win-back) |
...
const EVENT_TO_STATUS: Record<string, Status> = {
  ...
  SUBSCRIPTION_CANCELLATION: 'cancelled', PURCHASE_EXPIRED: 'cancelled', // cancelación / expiró tras reintentos
