...
-- `for all` SIEMPRE con `using` + `with check` (el hueco clásico que 25 documenta: sin
-- with check, un INSERT/UPDATE podría escribir filas con user_id ajeno):
create policy "own_progress" on user_progress
  for all using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);  -- patrón de alto rendimiento (ver 25)
...
create policy "own_achievements" on user_achievements
  for all using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);  -- mismo cierre del hueco de with check
```
