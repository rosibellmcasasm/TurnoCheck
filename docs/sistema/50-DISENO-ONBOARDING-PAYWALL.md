...
(3bis) PLAN CON NOMBRE PROPIO — el plan generado se presenta como ARTEFACTO con identidad,
    ...
    ⚠️ REQUIERE que el quiz haya pedido el NOMBRE: si usas este patrón, la pregunta de
    nombre es OBLIGATORIA en el onboarding (input libre de A3, ~pantalla 10-12 — ya hay
    confianza construida, no en la pregunta 1). Si no hay nombre, nombrar el plan por la
    META ("El Plan 5 kg — 4 semanas"), nunca un "El Plan [Nombre]" con placeholder vacío.
...
(9) TRUST ROW: garantia de devolucion + pago seguro + dato social real si existe. Íconos SVG
