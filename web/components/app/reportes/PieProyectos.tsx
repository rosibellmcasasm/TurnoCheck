...
export interface RebanadaProyecto {
  nombre: string;
  horas: number;
  color?: string | null;
}

const COLORES_RESPALDO = ["#2554C7", "#5B85E0", "#8FB0EE", "#B7CCF4", "#1E824C", "#B4790F", "#7C3AED", "#DB2777"];

export function PieProyectos({ datos }: { datos: RebanadaProyecto[] }) {
      ...
      <div className="relative h-32 w-32 shrink-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            >
              {datos.map((d, i) => (
                <Cell key={d.nombre} fill={d.color ?? COLORES_RESPALDO[i % COLORES_RESPALDO.length]} />
              ))}
            </Pie>
            <Tooltip
        ...
        {datos.slice(0, 6).map((d, i) => (
          <div key={d.nombre} className="flex items-center gap-1.5 text-xs">
            <span
              ...
              style={{ backgroundColor: d.color ?? COLORES_RESPALDO[i % COLORES_RESPALDO.length] }}
            />
